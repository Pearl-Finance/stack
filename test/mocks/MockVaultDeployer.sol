...
import {MockVaultImplementationDeployer} from "./MockVaultImplementationDeployer.sol";
 ...
 * @dev Utilizes the OpenZeppelin ERC1967 proxy pattern for deploying upgradable vaults using the MockStackVault
 * contract.
 ...
 */
contract MockVaultDeployer is CommonErrors, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function deployVault(
        ...
        bytes memory init = abi.encodeWithSelector(
            MockStackVault.initialize.selector,
        ...
        MockStackVault _vault = _deployImplementation(borrowToken, collateralToken);
    ...
    function upgradeVault(address payable proxy, address transferOwnershipTo) external {
        ...
        address borrowToken = address(MockStackVault(proxy).borrowToken());
        address collateralToken = address(MockStackVault(proxy).collateralToken());
    ...
    function _deployImplementation(address borrowToken, address collateralToken) internal returns (MockStackVault) {
        bytes memory result = implementationDeployer.functionDelegateCall(
            abi.encodeCall(MockVaultImplementationDeployer.deploy, (factory, borrowToken, collateralToken))
        );
        address vaultAddress = abi.decode(result, (address));
        return MockStackVault(payable(vaultAddress));
    }
