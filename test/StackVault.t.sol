...
contract StackVaultTest is Test {
    ...
    // function testShouldBorrowForMultipleUser(uint256 collateral, address addr, uint256 amountToBorrow) public {
    //     // stateful fuzzing logic, used for stateless fuzzing
    //     uint256 maxBorrowAmount = 1_000e18;
    //     uint256 minimumBorrowAmount = 1e18;
    //     uint256 borrowed;

    //     if (addr != address(0) && addr != address(1)) {
    //         vault.transferOwnership(addr);
    //         collateral = bound(collateral, 0, 1_00e18);
    //         if (collateral == 0) collateral = 1_00e18;

    //         amountToBorrow = bound(amountToBorrow, 0, collateral);
    //         if (amountToBorrow == 0) amountToBorrow = collateral / 2;

    //         if (borrowed + amountToBorrow > maxBorrowAmount) {
    //             if (borrowed > amountToBorrow) {
    //                 amountToBorrow = borrowed - amountToBorrow;
    //             } else {
    //                 amountToBorrow = maxBorrowAmount - borrowed;
    //             }
    //         }

    //         deal(address(collateralToken), addr, collateral);

    //         vm.startPrank(addr);
    //         collateralToken.approve(address(vault), collateral);
    //         (, uint256 share_) = totalCollateralAmount.add(collateral, Math.Rounding.Floor);
    //         uint256 share = vault.depositCollateral(addr, collateral);
    ...
    //         if (amountToBorrow >= (collateral * 90) / 100) amountToBorrow = (collateral * 89) / 100;
    //         if (amountToBorrow < minimumBorrowAmount) amountToBorrow = 0;

    //         if (amountToBorrow != 0) {
    //             uint256 balanceBefore = borrowToken.balanceOf(addr);
    //             uint256 feeReceieverBalanceBefore = borrowToken.balanceOf(address(1));

    //             uint256 userBorrowerSharesBefore = vault.userBorrowShare(addr);
    //             uint256 fee = amountToBorrow * vault.borrowOpeningFee() / 1e18;

    //             (, uint256 share_) = totalBorrowAmount.add(amountToBorrow + fee, Math.Rounding.Ceil);
    //             uint256 borrowShare = vault.borrow(addr, amountToBorrow);

    //             uint256 balanceAfter = borrowToken.balanceOf(addr);
    //             uint256 amountWithFee = amountToBorrow * vault.borrowOpeningFee() / 1e18 + amountToBorrow;

    //             assertEq(borrowShare, share_, "borrow share assertion");
    //             assertEq(balanceAfter - balanceBefore, amountToBorrow, "user assertion");

    //             assertEq(fee + feeReceieverBalanceBefore, borrowToken.balanceOf(address(1)), "fee receiever
    // assertion");
    //             assertEq(
    //                 borrowShare + userBorrowerSharesBefore,
    //                 vault.userBorrowShare(addr),
    //                 " user borrower shares assertion"
    //             );

    //             (uint256 collateralAmount, uint256 collateralValue, uint256 borrowAmount, uint256 borrowValue) =
    //                 vault.userPositionInfo(addr);

    //             assertEq(borrowAmount, amountWithFee, " borrow amount with fee assertion");
    //             assertEq(borrowValue, amountWithFee, " borrow value with fee assertion");
