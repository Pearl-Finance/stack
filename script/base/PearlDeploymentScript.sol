 ...
 * ('pearl.deployment') for CREATE2 address calculations. This salt ensures deterministic address generation for
 ...
 * needs. It uses the 'pearl.deployment' salt for all CREATE2 address calculations, providing consistency and
 * predictability in contract addresses.
 *
 * Key Features:
 * - Inherits the robust deployment and proxy management system of DeploymentScriptBase.
 * - Uses a specific salt ('pearl.deployment') to ensure deterministic and consistent CREATE2 address generation.
...
abstract contract PearlDeploymentScript is DeploymentScriptBase {
    constructor() DeploymentScriptBase("pearl.deployment") {}
}
